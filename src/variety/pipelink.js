...
(function(pidlist, classbase) {
	...
	"MouseEvent@pipelink": {
		inputModes: {
			edit: ["quesmark", "quesmark-", "info-line"],
			play: ["line", "peke", "info-line"]
		}
	},
	"MouseEvent@pipelinkr": {
		inputModes: {
			edit: ["quesmark", "quesmark-", "ice", "info-line"],
			play: ["line", "peke", "info-line"]
		}
	},
	"MouseEvent@loopsp": {
	...
	KeyEvent: {
		...
		key_inputLineParts: function(ca) {
			...
			if (ca === "q") {
				...
				if (this.pid !== "loopsp") {
					if (ca === "-") {
						cell.setQues(cell.ques !== -2 ? -2 : 0);
					} else if (this.pid === "pipelinkr" && ca === "1") {
						cell.setQues(6);
					} else {
						return;
					}
	...
	Cell: {
		...
		prehook: {
			ques: function(num) {
				this.setCombinedLine(num);
				return false;
			}
		},
		setCombinedLine: function() {
			// cell.setQuesから呼ばれる
			var bx = this.bx,
				by = this.by;
			...
			for (var i = 0; i < blist.length; i++) {
				var border = blist[i];
				if (border.line === 0 && border.isLineEX()) {
					border.setLineVal(1);
				}
	...
	Border: {
		...
		checkStableLine: function(num) {
			...
			return (num !== 0 && this.isLineNG()) || (num === 0 && this.isLineEX());
		},
		isLineEX: function() {
			var cell1 = this.sidecell[0],
				cell2 = this.sidecell[1];
			return this.isVert()
				? cell1.isLP(cell1.RT) && cell2.isLP(cell2.LT)
	...
	BoardExec: {
		adjustBoardData: function(key, d) {
			if (key & this.TURNFLIP) {
				...
				switch (key) {
					case this.FLIPY:
						tques = { 14: 17, 15: 16, 16: 15, 17: 14 };
						break;
					case this.FLIPX:
						tques = { 14: 15, 15: 14, 16: 17, 17: 16 };
						break;
					case this.TURNR:
						tques = { 12: 13, 13: 12, 14: 17, 15: 14, 16: 15, 17: 16 };
						break;
					case this.TURNL:
						tques = { 12: 13, 13: 12, 14: 15, 15: 16, 16: 17, 17: 14 };
						break;
				}
	...
	LineGraph: {
		...
		isedgevalidbylinkobj: function(border) {
			return border.isLine() || border.isLineEX();
		},
		...
		decodePipelink: function() {
			...
			for (var i = 0; i < bstr.length; i++) {
				...
				if (ca === ".") {
					...
					bd.cell[c].ques = parseInt(ca, 36) + 1;
				} else if (ca >= "h" && ca <= "z") {
					c += parseInt(ca, 36) - 17;
				}
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (qu === -2) {
					...
					if (type === parser.URL_PZPRV3) {
						for (var n = 1; n < 10; n++) {
							if (!bd.cell[c + n] || bd.cell[c + n].ques !== 6) {
								break;
							}
						}
						pstr = (n - 1).toString(10);
						c = c + n - 1;
					}
				} else if (qu >= 11 && qu <= 17) {
					pstr = (qu - 1).toString(36);
				} else {
					count++;
				}
				...
				if (count === 0) {
					cm += pstr;
				} else if (pstr || count === 19) {
					cm += (16 + count).toString(36) + pstr;
					count = 0;
				}
			}
			if (count > 0) {
				cm += (16 + count).toString(36);
			}
	...
	"Encode@loopsp": {
		...
		decodeLoopsp: function() {
			...
			for (var i = 0; i < bstr.length; i++) {
				...
				if (this.include(ca, "0", "9") || this.include(ca, "a", "f")) {
					...
					cell.qnum = parseInt(bstr.substr(i + 1, 3), 16);
					i += 3;
				} else if (ca >= "g" && ca <= "m") {
					cell.ques = parseInt(ca, 36) - 5;
				} else if (ca >= "n" && ca <= "z") {
					c += parseInt(ca, 36) - 23;
				}
		...
		encodeLoopsp: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (count === 0) {
					cm += pstr;
				} else if (pstr || count === 13) {
					cm += (22 + count).toString(36) + pstr;
					count = 0;
				}
			}
			if (count > 0) {
				cm += (22 + count).toString(36);
			}
		...
		encodePipelink: function() {
			this.encodeCell(function(cell) {
				if (cell.ques === 6) {
					...
					return "- ";
				} else if (cell.ques >= 11 && cell.ques <= 17) {
					return "" + (cell.ques - 1).toString(36) + " ";
				} else if (cell.qnum !== -1) {
					return cell.qnum + " ";
				} else {
					return ". ";
	...
	"AnsCheck@pipelinkr": {
		checkCrossOutOfMark: function() {
			this.checkAllCell(function(cell) {
				return cell.lcnt === 4 && cell.ques !== 6 && cell.ques !== 11;
			}, "lnCrossExIce");
		}
	},
	"CheckInfo@pipelinkr": {
