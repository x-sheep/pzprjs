...
(function(pidlist, classbase) {
	...
	BoardExec: {
		...
		getTranslateBits: function(key) {
			...
			switch (key) {
				case this.FLIPY:
					trans = { 2: 8, 3: 9, 6: 12, 7: 13, 8: 2, 9: 3, 12: 6, 13: 7 };
					break;
				case this.FLIPX:
					trans = { 1: 4, 3: 6, 9: 12, 11: 14, 4: 1, 6: 3, 12: 9, 14: 11 };
					break;
				case this.TURNR:
				case this.TURNL:
					// Transpose
	...
	"Encode@curvedata": {
		...
		encodePzpr: function(type) {
			...
			if (
				this.board.border.some(function(cell) {
					return cell.ques === 1;
				})
			) {
				this.outbstr += "b";
	...
	"AnsCheck@curvedata": {
		...
		checkShapes: function() {
			...
			for (var id = 0; id < components.length; id++) {
				var path = components[id];
				if (path.clueid === null || path.clueid === -2) {
					continue;
				}
		...
		shapeMatches: function(path) {
			...
			if (path.isomorphicWith === path.clueid) {
				return true;
			}
