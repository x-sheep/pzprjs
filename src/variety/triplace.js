...
(function(pidlist, classbase) {
	...
	Cell: {
		...
		getmaxnum: function() {
			var bd = this.board,
				target = this.puzzle.cursor.detectTarget(this);
			return target === this.RT
				? bd.cols - (this.bx >> 1) - 1
				: bd.rows - (this.by >> 1) - 1;
		},
		minnum: 0,
	...
	Border: {
		prehook: {
			qans: function() {
				return (
					this.sidecell[0].isnull ||
					this.sidecell[1].isnull ||
					this.sidecell[0].ques === 51 ||
					this.sidecell[1].ques === 51
				);
			}
		}
	},
		...
		decodeTriplace: function() {
			...
			for (var i = 0; i < bstr.length; i++) {
				...
				if (ca >= "g" && ca <= "z") {
					...
					if (ca === "_") {
						...
						cell.qnum = parseInt(bstr.charAt(i + 1), 36);
						i++;
					} else if (ca === "-") {
						cell.qnum2 =
							bstr.charAt(i + 1) !== "."
								? parseInt(bstr.charAt(i + 1), 16)
								: -1;
						...
						cell.qnum2 =
							bstr.charAt(i) !== "." ? parseInt(bstr.charAt(i), 16) : -1;
						cell.qnum =
							bstr.charAt(i + 1) !== "."
								? parseInt(bstr.charAt(i + 1), 16)
								: -1;
			...
			for (var i = a; i < bstr.length; i++) {
				...
				if (ca === ".") {
					...
					excell.qnum2 = parseInt(bstr.substr(i + 1, 2), 16);
					i += 2;
				} else {
					excell.qnum2 = parseInt(ca, 16);
				}
				id++;
				if (id >= bd.cols) {
					a = i + 1;
			...
			for (var i = a; i < bstr.length; i++) {
				...
				if (ca === ".") {
					...
					excell.qnum = parseInt(ca, 16);
				}
				id++;
				if (id >= bd.cols + bd.rows) {
					a = i + 1;
					break;
				}
			}
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (cell.ques === 51) {
					if (cell.qnum === -1 && cell.qnum2 === -1) {
						...
						if (cell.qnum >= 16 && cell.qnum2 >= 16) {
							pstr = "=" + pstr;
						} else if (cell.qnum >= 16) {
							pstr = "-" + pstr;
						} else if (cell.qnum2 >= 16) {
							pstr = "+" + pstr;
						}
					}
				} else {
